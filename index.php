...
<section class="articles-section">
    <div class="container pb-4">
        <div class="row g-4">
            <?php if (!empty($latestArticles) && is_array($latestArticles)): ?>
                <?php foreach ($latestArticles as $article): ?>
                    <div class="col-md-4">
                        <div class="card h-100 article-card">
                            <img src="<?= '/src/uploads/' . htmlspecialchars($article['urlPhotArt'] ?? '') ?>" class="card-img-top" alt="<?= htmlspecialchars($article['libTitrArt'] ?? '') ?>">
                            <div class="card-body d-flex flex-column">
                                <h5 class="card-title"><?= htmlspecialchars($article['libTitrArt'] ?? '') ?></h5>
                                <p class="card-text">
                                    <?= substr(htmlspecialchars($article['libChapoArt'] ?? ''), 0, 100) . '...' ?>
                                </p>
                                <div class="article-meta mt-auto pt-3 d-flex justify-content-between align-items-center">
                                    <span class="article-date">
                                        Posté le <?= !empty($article['dtCreaArt']) ? date("d/m/Y", strtotime($article['dtCreaArt'])) : '' ?>
                                    </span>
                                    <div class="article-icons">
                                        <a href="views/frontend/articles/article1.php?idArt=<?= urlencode($article['numArt'] ?? '') ?>" class="text-dark">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 d-flex align-items-end justify-content-lg-end mt-4 mt-lg-0">
                            <div class="feature-btn-wrapper">
                                <a href="/views/frontend/original.php" class="btn-read-feature">Lire plus &rarr;</a>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="container">
        <div class="row align-items-center">
            ...
            <div class="col-lg-6 ps-lg-5">
                ...
                <div class="mt-4">
                    <a href="views/backend/security/signup.php" class="btn-inscription">Inscription</a>
                </div>
            </div>
        </div>
    </div>
